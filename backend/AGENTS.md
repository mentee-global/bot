...
- `app/api/deps.py` — process-wide singletons for store / agent / service / sessions, plus `require_session` / `optional_session` cookie dependencies.
...
This backend also vendors the Pydantic AI agent-building skill from `pydantic/skills`; read it before replacing or extending the mock agent with Pydantic AI:

- `.agents/skills/building-pydantic-ai-agents/SKILL.md` — agents, tools, structured output, streaming, testing, capabilities, hooks, and multi-agent patterns
- `.agents/skills/building-pydantic-ai-agents/references/` — focused references for implementation details
