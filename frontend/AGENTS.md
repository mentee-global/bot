...
- **Color & contrast (must read — recurring source of bugs).** Always pick a text color that contrasts the surface it sits on, and check it in BOTH light and dark (the same token flips value between themes).
  - Text on the neutral surface/background: `--theme-primary` (body), `--theme-secondary`, `--theme-muted` (captions). For a destructive label use `--theme-danger` (the red itself).
  - `*-fg` / `*-on-*` tokens (`--theme-danger-fg`, `--theme-on-accent`) are the text color for content sitting ON that colored FILL — never use them as text on the neutral surface (they're white/black and vanish). Pair them only with their matching `bg` (e.g. `bg-[--theme-accent] text-[--theme-on-accent]`, `bg-[--theme-danger] text-[--theme-danger-fg]`).
  - **Links inside a colored container**: the global `a {}` rule forces `color: var(--theme-primary)`, so an `<a>` on a `--theme-primary` (or accent) background renders primary-on-primary and disappears. Add `text-inherit hover:text-inherit` (or an explicit contrasting token) to any link inside a colored bubble/fill.
  - Don't derive a chip/badge's text and its background from the same token at different opacities — they won't contrast. Inherit the container's text color and give the pill a faint fill.
  - When building or changing any UI, verify contrast in light AND dark (toggle the theme); the `web-design-guidelines` skill reviews this if unsure.
- **React Compiler**: enabled in `vite.config.ts` via `@rolldown/plugin-babel` + `reactCompilerPreset()` (exported from `@vitejs/plugin-react`). v6 of `@vitejs/plugin-react` dropped the `babel` option since Vite 8 + Oxc handle JSX/Fast Refresh natively, so the compiler now plugs in as a separate babel-stage plugin. Don't hand-apply `useMemo`/`useCallback` for the compiler's sake.
- **Vite plugin order** matters: `devtools()` stays first, then `paraglideVitePlugin`, `tailwindcss`, `tanstackStart`, `nitro`, `babel` (with `reactCompilerPreset`), `viteReact`. The babel plugin **must** run before `viteReact`.
